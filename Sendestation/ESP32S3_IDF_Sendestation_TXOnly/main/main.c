...
#define CRSF_RX_PIN                 GPIO_NUM_17
...
static void fill_channels(uint16_t *channels)
{
    ...
    channels[3] = bool_to_crsf(s_state.button[0]);
    channels[4] = bool_to_crsf(s_state.button[1]);
    channels[5] = bool_to_crsf(s_state.button[8]);
    channels[6] = bool_to_crsf(s_state.button[9]);
    channels[7] = bool_to_crsf(s_state.button[10]);
    channels[8] = bool_to_crsf(s_state.button[11]);
    channels[9] = bool_to_crsf(s_state.button[12]);
...
static bool decode_host_packet(const uint8_t *packet, steuerdaten_t *out_state)
{
    ...
    out_state->lenkung_us = norm_steer_to_us((int16_t)(packet[2] | (packet[3] << 8U)));
    out_state->gas_us = norm_pedal_to_us((uint16_t)(packet[4] | (packet[5] << 8U)));
    out_state->bremse_us = norm_pedal_to_us((uint16_t)(packet[6] | (packet[7] << 8U)));
...
static void init_host_uart(void)
{
    ...
    ESP_ERROR_CHECK(uart_driver_install(UART_HOST, 512, 0, 0, NULL, 0));
...
static void init_crsf_uart(void)
{
    ...
    ESP_ERROR_CHECK(uart_driver_install(UART_CRSF, 256, 0, 0, NULL, 0));
    ...
    ESP_ERROR_CHECK(uart_set_mode(UART_CRSF, UART_MODE_RS485_HALF_DUPLEX));
}
...
void app_main(void)
{
    ...
    while (1) {
        ...
        if (now_us >= next_send_us) {
            ...
            uart_write_bytes(UART_CRSF, s_tx_frame, sizeof(s_tx_frame));
        }
